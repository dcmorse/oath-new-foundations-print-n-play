...
# generator
src_dim = ((i,j) for j in src_dims[1] for i in src_dims[0])
...
def is_red_triangle_card(src_img, card_idxs) -> float:
    ...
    def count_red_pixels(offset_pxs: List[Tuple[int]]):
        ...
        for (rx, ry) in offset_pxs:
            ...
            distance_from_red_squared = sum((c1 - c0)**2 for (c1, c0) in zip(pixel_value, red))
            if distance_from_red_squared <= 300:
                count += 1
        return count
    confidence = count_red_pixels(expect_red_pxs) + 3 - count_red_pixels(expect_not_red_pxs)
...
def requilt():
    ...
    dst_file_idx = 0
    def write_page():
        fname = f"quilt-{dst_file_idx:02}.png"
        print("write_page()")
        dst_img.save(fname)
